...
const Card = (props: any) => {
    ...
    const handleSaveJob = () => {
        ...
        if (savedJobs.includes(props.id)) {
            updatedJobs = updatedJobs.filter((jobId) => jobId !== props.id);
        } else {
            updatedJobs.push(props.id);
        }
    ...
    return (
        <div data-aos="zoom-out" className="p-4 rounded-xl bg-mine-shaft-900 hover:shadow-[0_0_5px_1px_yellow] !shadow-bright-sun-400 transition duration-300 ease-in-out w-72 flex flex-col gap-3">
            <div className="flex justify-between">
                <div className="flex gap-2 items-center">
                    <div className="p-2 bg-mine-shaft-800 rounded-md">
                        <img className="h-7" src={`/Icons/${props.company}.png`} alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
                    </div>

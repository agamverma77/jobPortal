...
const JobCategory = () => {
  return (
    <div className="mt-20 pb-5 overflow-hidden">
      <div
        data-aos="zoom-out"
        className="text-4xl text-center font-semibold md-mx:text-3xl sm-mx:text-2xl xs-mx:text-xl mb-3 text-mine-shaft-100"
      >
        Browse <span className="text-bright-sun-400">Job </span>Category
      </div>
      <div
        data-aos="zoom-out"
        className="text-lg sm-mx:text-base xs-mx:text-sm mb-10 mx-auto text-mine-shaft-300 text-center w-1/2 sm-mx:w-11/12"
      >
      ...
      <Carousel
        slideSize="22%"
        slideGap="md"
        emblaOptions={{ loop: true }} // ✅ Corrected here
      ...
      >
        {jobCategory.map((category, index) => (
          <Carousel.Slide key={index}>
            <div
              data-aos="zoom-out"
              className="flex flex-col items-center w-64 sm-mx:w-56 xs-mx:w-48 gap-2 border border-bright-sun-400 p-5 rounded-xl hover:cursor-pointer hover:shadow-[0_0_5px_2px_black] my-5 transition duration-300 ease-in-out !shadow-bright-sun-300"
            >
